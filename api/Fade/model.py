...
from Fade.MIDAS.anom import midas_u
import pandas as pd
...
class Model(nn.Module):
  ...
  def forward(self, s_id, d_id):
    #On y décrit le traitement à effectuer de l'entrée à la sortie
    # In the forward function we accept a Tensor of input data and we must return  a Tensor of output data. We can use Modules defined in the constructor as
        # well as arbitrary operators on Tensors.
    h_s = self.h_static[s_id.type(torch.long), :]
    h_d = self.h_static[d_id.type(torch.long), :]
...
def F_FADE(model, dataset, t_setup, W_upd, alpha, M, T_th, epochs, online_train_steps, max_batch_size, device, num_rows=15, num_buckets=10, m=18, seuil_f=1000.0):
  # Step 1
  ...
  for t in tqdm(range(1, T_max + 1)):
    ...
    while edge_stream[data_idx].curr_time == t:
      ...
      current_df = current_df.append([[s, d,t]])
      data_idx += 1
    ...
    if t > t_setup:
     ...
     score = midas_u(current_df,t,score,cur_count,total_count)
     score_mi = set(score)
     for s in score_mi:
       if s[2] > seuil_f:
        print("deleted")
        if (s[0], s[1])  in delta_F:
          del delta_F[(s[0], s[1])]
